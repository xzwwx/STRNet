...
class Res2Net(nn.Module):
    ...
    def __init__(self, block, layers, baseWidth = 26, scale = 4, num_classes=101):
        self.inplanes = 64
        ...
        self.fc = nn.Linear(512 * block.expansion*self.num_frames, num_classes)
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x)
        x = x.view(int(x.size(0)/self.num_frames), -1)
        x = self.fc(x)
...
if __name__=='__main__':
    ...
    model = model.cuda()
    data = torch.autograd.Variable(torch.rand(2, num_frames, 18, 224, 224)).cuda()
    ...
    image = torch.rand(10,3,224,224).cuda()
